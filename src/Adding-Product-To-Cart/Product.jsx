...
<hr className="my-6 border-blue-500" />
        ...
        {cart.length === 0 ? (
            ...
            <ul className="grid gap-4 grid-cold-2 mt-3 shadow-md font-bold">
                {cart.map((item) => (
                    <li key={item.id} 
                    className="flex justify-between border border-blue-500 p-1 hover:bg-blue-600 hover:text-white transition duration">
                       <span className="text-blue-400">{item.name}</span> <span className="text-blue-400">{item.quantity}</span> <span className="text-blue-400"> ${item.price * item.quantity}</span>
                        <button onClick={() => RemoveProduct(item.id)}
                            className="bg-red-500 py-2 px-4 text-white rounded border border-red-700 hover:bg-red-600 transition duration-300">Remove</button>
                    </li>
                ))}
            </ul>
        )
        }
  <h3 className="mt-6">Total: ${getTotalPrice()}</h3>
    </div>
   )
}
